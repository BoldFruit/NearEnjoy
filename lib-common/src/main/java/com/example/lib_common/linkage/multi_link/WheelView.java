...
public class WheelView extends ScrollView {
    ...
    public interface OnWheelViewListener {
        void onSelected(int selectedIndex, String item, int mSeries, int id);
    }
    ...
    public void setRealItems(List<ContentWrapper> list) {
        if (null == items) {
            items = new ArrayList<String>();
        }
        if (null == ids) {
        ...
        for (ContentWrapper wrapper : list) {
            items.add(wrapper.getName());
...
//            Log.d(TAG, "向上滚动");
    ...
    private void onSeletedCallBack() {
        if (null != onWheelViewListener) {
            if (mSeries == -1) {
                throw new IllegalArgumentException("未设置级数");
            }

            onWheelViewListener.onSelected(selectedIndex, items.get(selectedIndex), mSeries, ids.get(selectedIndex - 1));
        }
