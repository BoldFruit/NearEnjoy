...
package com.example.lib_common.bottomnavi.tabstrip;
...
/**
 * 滑动渐变的底部Tab，支持中间添加View
 *
 * @author techflowing
 * @since 2018/9/16 上午1:21
 */
public class GradientTabStrip extends BaseTabStrip {
    ...
    public GradientTabStrip(Context context, AttributeSet attrs,
        ...
        mDrawableHeight = custom.getDimensionPixelOffset(
                R.styleable.GradientTabStrip_gtsDrawableHeight, mDrawableHeight);
        mDrawableWidth = custom.getDimensionPixelOffset(
                R.styleable.GradientTabStrip_gtsDrawableWidth, mDrawableWidth);
        custom.recycle();
    ...
    protected void drawInterval(Canvas canvas, int position) {
        if (mInterval == null || mInterval.getIntrinsicWidth() <= 0
            ...
            return;
        if (getItemCount() % 2 == 0 && position + 1 == getItemCount() / 2 && mCenterGap != 0)
            return;
...
//        float mX = ViewCompat.getPaddingStart(this) + mItemWidth * (position + 1) + getIntervalWidth() * position;
//        if (mCenterGap > 0 && position * 2 >= getItemCount() - 1 && getItemCount() % 2 == 0) {
//            mX = mX - getIntervalWidth() + mCenterGap;
//        }
//
//        final float moveX = mX;
        final float moveY = getPaddingTop() +
    ...
    protected void drawDrawable(Canvas canvas, int position) {
        ...
        if (normal != null) {
            final int normalWidth = mDrawableWidth != 0 ? mDrawableWidth : normal.getIntrinsicWidth();
            final int normalHeight = mDrawableHeight != 0 ? mDrawableHeight : normal.getIntrinsicHeight();
        ...
        if (selected != null) {
            final int selectedWidth = mDrawableWidth != 0 ? mDrawableWidth : selected.getIntrinsicWidth();
            final int selectedHeight = mDrawableHeight != 0 ? mDrawableHeight : selected.getIntrinsicHeight();
