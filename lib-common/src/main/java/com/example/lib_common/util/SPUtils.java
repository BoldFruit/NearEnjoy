...
public class SPUtils {
    ...
    public static <E>E get(@NonNull String key,@NonNull E defaultValue, Type mType) {
        ...
        //json为null的时候返回对象为null,gson已处理
        return (E) new Gson().fromJson(sp.getString(key, null),mType);
    }
     ...
     * 获取保存的List对象
     * @param tag
     * @param type eg object : TypeToken<List<ClassificationBean>>() {}.type,将ClassificationBean改为你需要的类型
     * @param <T>
     * @return
     */
    public static <T> List<T> getDataList(String tag, Type type) {
