...
import com.example.lib_common.linkage_kt.viewholder.MainGridViewHolder
 ...
 * Function:
 */
class MainGridAdapter(private var dataList: ArrayList<IBaseLinkData>, var itemClickListener: ContentAdapter.OnItemClickListener): RecyclerView.Adapter<RecyclerView.ViewHolder>() {

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        return when(val type: Int = dataList[viewType].getType()) {
            DataType.ITEM_DATA -> MainGridViewHolder(inflateView(parent, getLayout(type)))
    ...
    private fun getLayout(viewType: Int): Int {
        return when(viewType) {
            DataType.ITEM_DATA -> R.layout.common_linkdata_main_content_grid
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        ...
        if (holder is MainGridViewHolder) {
