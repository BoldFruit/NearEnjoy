package com.example.module_chat.chatbubble;
 ...
 * 自定义气泡的shape背景
 */
public class ChatShape extends Shape {
    /**
    ...
    /*以下是默认设置，修正值为3F*/
    private final float reviseValue = 3F; //向内修正值
    private final float widthStart = reviseValue;
    private final float heightStart = reviseValue;

    /*以下是绘制时需要使用的变量*/

    private float width,height;  //测量得到的view的长宽
    private float widthEnd ;   //减去修正值后，view右侧边界的坐标
    private float heightEnd ;  //减去修正值后，view底部边界的坐标

    private Path strokePath = new Path(); //边框path
    private Path fillPath = new Path();  //填充path
